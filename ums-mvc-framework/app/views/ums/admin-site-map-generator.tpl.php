<div class="container col-md-10 text-left">
    ...
    <?php endif; ?>
    <form id="site-map-generator-form" action="/ums/generator/site/map" method="post" class="p-3">
    	...
    	<?php foreach ($routes as $key => $route): ?>
        	<div id="form-route-<?=$key?>" class="form-group justify-content-center row input-route p-2">
        		...
        		<input id="route-<?=$key?>" name="route-<?=$key?>" value="/<?=$route ?? ''?>" placeholder="Route" class="form-control evidence-error send-ajax col-10" type="text" required="required">
        		<label for="lastmod-<?=$key?>" class="col-10 mt-3">Last modification</label>
        		<input id="lastmod-<?=$key?>" name="lastmod-<?=$key?>" placeholder="Last modification" class="form-control evidence-error send-ajax col-10" type="date">
        		<label for="priority-<?=$key?>" class="col-10 mt-3">Priority</label>
        		<input id="priority-<?=$key?>" name="priority-<?=$key?>" placeholder="Priority" class="form-control evidence-error send-ajax col-10" type="number" step="0.1" min="0.0" max="1.0">
        		...
        		<select id="changefreq-<?=$key?>" name="changefreq-<?=$key?>" class="evidence-error send-ajax col-10">
        			...
        			<?php foreach ($changefreqList as $changefreq): ?>
        				<option value="<?=$changefreq?>"><?=ucfirst($changefreq)?></option>
        			<?php endforeach; ?>
        		</select>
